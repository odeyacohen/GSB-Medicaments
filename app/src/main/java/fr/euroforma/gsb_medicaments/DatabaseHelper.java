...
public class DatabaseHelper extends SQLiteOpenHelper {
    ...
    private static final String PREMIERE_VOIE= "selectionner une voie d'administration";
    ...
    public List<String> getDistinctVoiesAdmin() {
        ...
        Cursor cursor = db.rawQuery("SELECT DISTINCT Voies_dadministration FROM CIS_bdpm WHERE Voies_dadministration NOT LIKE '%;%' order by 1", null);
    ...
    public List<Medicament> searchMedicaments(String denomination, String formePharmaceutique, String titulaires, String denominationSubstance, String voiesAdmin)
    {
        ...
        String SQLSubstance = "SELECT CODE_CIS FROM CIS_COMPO_bdpm join CIS_bdpm using (CODE_CIS) WHERE replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(replace(upper(Denomination_substance), 'Â','A'),'Ä','A'),'À','A'),'É','E'),'Á','A'),'Ï','I'), 'Ê','E'),'È','E'),'Ô','O'),'Ü','U'), 'Ç','C' ) LIKE ?"
                ;
//String SQLSubstance = "SELECT CODE_CIS FROM CIS_COMPO_bdpm WHERE Denomination_substance COLLATE latin1_general_cs_ai LIKE ?" ;

        // La requête SQL de recherche
        String query = "SELECT *,(select count(*) from CIS_COMPO_bdpm c where c.Code_CIS=m.Code_CIS) as nbMolecule FROM CIS_bdpm m WHERE " +
                "Denomination_du_medicament LIKE ? AND " +
                "Forme_pharmaceutique LIKE ? AND " +
                "Titulaires LIKE ? AND " +
                "Code_CIS IN (" + SQLSubstance + ") "+finSQL;
        ...
        if (cursor.moveToFirst()) {
            do {
                ...
                String statutAdministratif = cursor.getString(cursor.getColumnIndexOrThrow("Statut_administratif_de_lAMM"));
                String nbMolecule = cursor.getString(cursor.getColumnIndexOrThrow("nbMolecule"));
                ...
                medicament.setNbMolecule(nbMolecule);

                // Ajouter l'objet Medicament à la liste
                medicamentList.add(medicament);
            } while (cursor.moveToNext());
        }else{
            Toast.makeText(mycontext, "Aucun resultat", Toast.LENGTH_LONG).show();
